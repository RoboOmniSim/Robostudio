...
def read_with_new_fps(cap):
    ...
    # Interval between frames to capture, to achieve 2 fps
    interval = int(fps / 2)

    # Loop to read frames from start_frame to end_frame at 2 fps
    ...
    while current_frame <= end_frame:
        ...
        # Move to the next frame in the next iteration by skipping 'interval' frames
...
def infer(args: Namespace, model: BaseModel) -> None:
    ...
    if args.output_path.exists()==False:
        import os
    ...
    cap, img_paths, num_imgs, prev_img_raw = init_input(args.input_path)

    prev_img=cv.resize(prev_img_raw,(960,540))
    flow_gt = None
    ...
    for i in tqdm(range(1, num_imgs)):
        img_raw, img_dir_name, img_name, is_img_valid = _read_image(cap, img_paths, i)

        # downsample due to gpu size limit
        img=cv.resize(img_raw, (960, 540))
        ...
        if img_dir_name == prev_dir_name:
            ...
            preds_npy["resize_flows"] = cv.resize(preds_npy["flows"], (1920, 1080))
            preds_npy["images"] = cv.resize(preds_npy["images"], (1920, 1080))
            preds_npy["flow_small"] = cv.resize(preds_npy["flow_small"], (preds_npy["flow_small"].shape[1]*2, preds_npy["flow_small"].shape[0]*2))
            ...
            torch.save(preds_npy["resize_flows"], args.output_path / f"resize_flows_{img_name}.pth")
