...
from nerfstudio.models.splatfacto_backward import SplatfactobackwardModelConfig
from nerfstudio.data.dataparsers.nerfstudio_dataparser_backward import NerfstudioDataParserbackwardConfig
...
method_configs["splatfacto"] = TrainerConfig(
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            dataparser=NerfstudioDataParserConfig(downscale_factor=1,load_3D_points=True),
...
method_configs["splatfactobackward"] = TrainerConfig(
    method_name="splatfactobackward",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    steps_per_eval_all_images=1000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            # dataparser=NerfstudioDataParserConfig(auto_scale_poses=False,downscale_factor=1,load_3D_points=True), # raw scale
            dataparser=NerfstudioDataParserbackwardConfig(downscale_factor=1,load_3D_points=True),
            cache_images_type="uint8",
        ),
        model=SplatfactobackwardModelConfig(),
    ),
    optimizers={
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
